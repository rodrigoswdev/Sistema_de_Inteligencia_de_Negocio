...
export async function GET(request: Request) {
  ...
  const moduleName = (url.searchParams.get("module") ?? "VENTAS") as ReportModule;
  ...
  const workbook = createSpreadsheet(rows, {
    title: `SIBI CBN · Reporte ${moduleName}`,
  ...
  return new Response(new Uint8Array(workbook), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${moduleName.toLowerCase()}-sibi-cbn.xls"`,
    },
  });
}
